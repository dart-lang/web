...
RawType getRawType(Translator translator, idl.IDLType idlType) {
  ...
  if (alias == null && !translator.markTypeAsUsed(type)) {
    type =
        _getJSTypeEquivalent(translator, RawType(type, false))?.type ?? 'JSAny';
  }
...
RawType _computeRawTypeUnion(
  ...
  RawType? computeTypeParamUnion(RawType? typeParam1, RawType? typeParam2) =>
      ...
      ? _computeRawTypeUnion(translator, typeParam1, typeParam2)
      : null;
  ...
  final unionableType1 = _getJSTypeEquivalent(translator, rawType1) ?? rawType1;
  final unionableType2 = _getJSTypeEquivalent(translator, rawType2) ?? rawType2;
...
RawType? _getJSTypeEquivalent(Translator translator, RawType rawType) {
    ...
    switch (nodeType) {
      ...
      default:
        ...
        if (desugaredType != null) {
          return _getJSTypeEquivalent(translator, desugaredType) ??
              desugaredType;
        }
    }
  }

...
/// [event handler attributes](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes).
...
extension type Event._(JSObject _) implements JSObject {
  ...
  ///
  /// > [!NOTE]
  /// > Look for better alternatives than using `preventDefault()` to block
  /// > default actions. For example, you can use the `disabled` or `readonly`
  /// > attribute on a form control to prevent it from being interacted with,
  /// > use
  /// > [HTML constraint validation](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Constraint_validation)
...
extension type Node._(JSObject _) implements EventTarget, JSObject {
  ...
  /// > reference is kept,
  /// > it will be
  /// > [automatically deleted](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Memory_management)
...
/// [many others](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements).
/// It provides functionality globally to the document, like how to obtain the
...
extension type Document._(JSObject _) implements Node, JSObject {
  ...
  /// [declarative shadow roots](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/template#declarative_shadow_dom),
  ...
  /// > `<plaintext>` or `<!--`). In other cases, the call can clear the current
  /// > page first, as if
  ...
  /// [form inputs](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/input),
  ...
  /// currently active editable element can be modified, but some commands
  /// (e.g., `copy`) can work without an editable element.
  ///
  /// > [!NOTE]
  ...
  /// [Quirks mode](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Quirks_mode_and_standards_mode)
  /// or
  /// Standards mode.
  ...
  /// otherwise by a
  /// [Permissions-Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Permissions-Policy/picture-in-picture).
  ///
  /// Although this property is read-only, it will not throw if it is modified
  /// (even in
...
extension type ShadowRoot._(JSObject _) implements DocumentFragment, JSObject {
  ...
  /// [declarative shadow root](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/template#declarative_shadow_dom)
  ...
  /// [`shadowrootdelegatesfocus`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/template#shadowrootclonable)
  /// attribute of the
  /// [`<template>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/template)
  /// element when a shadow root is created declaratively.
...
extension type Element._(JSObject _) implements Node, JSObject {
  ...
  /// [`class` attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/class)
  ...
  /// The **`scrollTop`** property of the [Element] interface gets or sets the
  /// number of pixels by which an element's content is scrolled from its top
  /// edge. This value is subpixel precise in modern browsers, meaning that it
  /// isn't necessarily a whole number.
  external double get scrollTop;
  external set scrollTop(num value);

  /// The **`scrollLeft`** property of the [Element] interface gets or sets the
  /// number of pixels by which an element's content is scrolled from its left
  /// edge. This value is subpixel precise in modern browsers, meaning that it
  /// isn't necessarily a whole number.
  external double get scrollLeft;
  external set scrollLeft(num value);

  /// The **`scrollWidth`** read-only property of the [Element] interface is a
  ...
  /// [Element.clientWidth]: it includes the element's padding, but not its
  /// border, margin or vertical scrollbar (if present). It can also include the
  /// width of pseudo-elements such as  or . If the element's content can fit
  /// without a need for horizontal scrollbar, its `scrollWidth` is equal to
  /// [Element.clientWidth].
  external int get scrollWidth;

  /// The **`scrollHeight`** read-only property of the [Element] interface is a
  ...
  /// require in order to fit all the content in the viewport without using a
  /// vertical scrollbar. The height is measured in the same way as
  /// [Element.clientHeight]: it includes the element's padding, but not its
  /// border, margin or horizontal scrollbar (if present). It can also include
  /// the height of pseudo-elements such as  or . If the element's content can
  /// fit without a need for vertical scrollbar, its `scrollHeight` is equal to
  /// [Element.clientHeight].
  external int get scrollHeight;

  /// The **`clientTop`** read-only property of the [Element] interface returns
  /// the width of the top border of an element in pixels.
  ///
  /// All that lies between the `offsetTop` and `clientTop` is the element's
  /// border. This is because the `offsetTop` indicates the location of the top
  /// of the border (not the margin) while the client area starts immediately
  /// below the border, including padding. Therefore, the `clientTop` value is
  /// always equal to the `border-top-width`, rounded to integer. For example,
  /// if the computed `border-top-width` is zero, then `clientTop` is also zero.
  external int get clientTop;

  /// The **`clientLeft`** read-only property of the [Element] interface returns
  /// the width of the left border of an element in pixels. It includes the
  /// width of the vertical scrollbar if the text direction of the element is
  /// right-to-left and if there is an overflow causing a left vertical
  /// scrollbar to be rendered. `clientLeft` does not include the left margin or
  /// the left padding.
  ///
  /// > [!NOTE]
  /// > When an element has `display: inline`, `clientLeft` returns `0`
  /// > regardless of the element's border.
  external int get clientLeft;

  /// The **`clientWidth`** read-only property of the [Element] interface is
  /// zero for inline elements and elements with no CSS; otherwise, it's the
  /// inner width of an element in pixels. It includes padding but excludes
  ...
  /// The **`clientHeight`** read-only property of the [Element] interface is
  /// zero for elements with no CSS or inline layout boxes; otherwise, it's the
  /// inner height of an element in pixels. It includes padding but excludes
  /// borders, margins, and horizontal scrollbars (if present).
  ///
  /// `clientHeight` can be calculated as: CSS `height` + CSS `padding` - height
  /// of horizontal scrollbar (if present).
  ///
  /// When `clientHeight` is used on the root element (the `<html>` element),
  /// (or on `<body>` if the document is in quirks mode), the viewport's height
  /// (excluding any scrollbar) is returned.
  ...
  /// The **`role`** property of the [Element] interface returns the explicitly
  /// set
  /// [WAI-ARIA role](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles)
  /// for the element.
  ///
  /// All HTML elements have an implicit ARIA role, even if that role is
  /// [`generic`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/generic_role).
  /// This semantic association allows tools to present and support interaction
  /// with the object in a manner that is consistent with user expectations
  /// about other objects of that type. The `role` attribute is used to
  /// explicitly set the element's ARIA role, overriding the implicit role. For
  /// example, a `ul`, which has an implicit
  /// [`list`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/list_role)
  /// role, might have
  /// [`role="treegrid"`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/treegrid_role)
  /// explicitly set. The `role` property reflects the explicitly set value of
  /// the `role` attribute—in this case `treegrid`; it does not return the
  /// element's implicit `list` role unless explicitly set.
  ///
  /// The full list of defined ARIA roles can be found on the
  ...
  /// The **`ariaInvalid`** property of the [Element] interface reflects the
  /// value of the
  ...
  ///
  /// If the attribute is not present, or is set to the empty string, assistive
  /// technology will treat the value as if it were set to `false`. If the
  /// attribute is present but set to a value other than `false`, `grammar`,
  /// `spelling` or the empty string (`""`), assistive technology treats the
  /// value as `true`. The property reflects the attribute value as set, not as
  /// handled by assistive technology.
  ...
  /// expect from the
  /// [live region](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Guides/Live_regions).
...
extension type Range._(JSObject _) implements AbstractRange, JSObject {
  ...
  /// The **`compareBoundaryPoints()`** method of the [Range] interface compares
  /// the boundary points of the [Range] with those of another range.
  external int compareBoundaryPoints(
    int how,
    Range sourceRange,
  );

  /// The **`Range.deleteContents()`** method removes all completely-selected
  /// [Node] within this range from the document. For the partially selected
  /// nodes at the start or end of the range, only the selected portion of the
  /// text is deleted, while the node itself remains intact. Afterwards, the
  /// range is collapsed to the end of the last selected node.
  ///
  /// ```plain
  ...
  /// The **`extractContents()`** method of the [Range] interface is similar to
  /// a combination of [Range.cloneContents] and [Range.deleteContents]. It
  /// removes the child [Node]s of the range from the document, clones them, and
  /// returns them as a new [DocumentFragment] object. For partially selected
  /// nodes, only the selected text is deleted, but all containing parent nodes
  /// up to the common ancestor are cloned as well, resulting in two copies of
  /// these nodes, one in the original document and one in the extracted
  /// fragment.
  external DocumentFragment extractContents();

  /// The **`cloneContents()`** method of the [Range] interface copies the
  /// selected [Node] children of the range's [Range.commonAncestorContainer]
  /// and puts them in a new [DocumentFragment] object.
  ///
  /// Nodes are cloned using the same algorithm as [Node.cloneNode], which means
  /// event listeners attached with scripts are not cloned. HTML `id` attributes
  /// are cloned, which can lead to an invalid document through cloning.
  ///
  /// The first and last selected children of `commonAncestorContainer` may be
  /// partially selected. In this case, the child node itself is cloned, but its
  /// content is only the part that is selected, by recursively cloning the
  /// range between the original range's start/end boundary and that child
  /// node's end/start boundary.
  ///
  /// ```plain
  /// <p>paragraph 1</p><p>paragraph 2</p><p>paragraph 3</p>
  ///        ^----------- selection ------------^
  ...
  /// The **`surroundContents()`** method of the [Range] interface surrounds the
  /// selected content by a provided node. It
  /// [extracts](https://developer.mozilla.org/en-US/docs/Web/API/Range/extractContents)
  /// the contents of the range, replaces the children of `newParent` with the
  /// extracted contents,
  /// [inserts](https://developer.mozilla.org/en-US/docs/Web/API/Range/insertNode)
  /// `newParent` at the location of the extracted contents, and makes the range
  /// select `newParent`.
  ///
  /// An exception is thrown if the range partially contains any non-[Text]
  /// node. The range must only contain text nodes and completely selected
  /// nodes.
  ...
  /// The **`isPointInRange()`** method of the [Range] interface determines
  /// whether a specified point is within the [Range]. The point is specified by
  /// a reference node and an offset within that node. It is equivalent to
  /// calling [Range.comparePoint] and checking if the result is `0`.
  external bool isPointInRange(
    Node node,
    int offset,
  );

  /// The **`comparePoint()`** method of the [Range] interface determines
  /// whether a specified point is before, within, or after the [Range]. The
  /// point is specified by a reference node and an offset within that node.

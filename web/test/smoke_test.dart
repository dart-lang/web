...
void main() {
  ...
  test('URLSearchParams asMap', () {
    ...
    // Sanity checks on Object? types for map operations
    map['a'] = '1';
    final dummyJsObject = JSObject();
    final dummyDartObject = Object();

    // operator [] should safely return null for non-Strings
    // ignore: collection_methods_unrelated_type
    expect(map[dummyJsObject], isNull);
    expect(map[dummyDartObject], isNull);
    // ignore: collection_methods_unrelated_type
    expect(map[123], isNull);

    // remove should safely return null for non-Strings
    // ignore: collection_methods_unrelated_type
    expect(map.remove(dummyJsObject), isNull);
    expect(map.remove(dummyDartObject), isNull);
    // ignore: collection_methods_unrelated_type
    expect(map.remove(123), isNull);

    // Ensure actual element wasn't removed accidentally
    expect(map['a'], '1');
  });

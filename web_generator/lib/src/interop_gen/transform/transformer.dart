...
class Transformer {
  ...
  DeclarationMap filter([bool Function(Decl)? saveIf]) {
    ...
    declarationMap.forEach((k, v) {
      ...
      switch (v) {
        case final Exportable e:
          if (e.exported) {
            filteredDeclarations[(e as Decl).id] = e as Decl;
          }
          break;
        case final PrimitiveType t:
          // primitive types are generated by default
          filteredDeclarations.add(t);
          break;
        case Type():
  ...
  DeclarationMap _filterForDecl([Decl? decl]) {
    final filteredDeclarations = DeclarationMap({});

    switch (decl) {
      case final VariableNode v:
        filteredDeclarations.add(v.type);
        break;
      case final UnionType u:
        filteredDeclarations.addAll({for (final t in u.types) t.id: t});
        break;
  ...
  List<Node> _transformVariable(TSVariableStatement variable) {
  ...
  Type _transformType(TSTypeNode type) {
    ...
    if (type.kind == TSSyntaxKind.TypeReference) {
      ...
      final firstNode = declarationsMatching.whereType<Node>().first;
